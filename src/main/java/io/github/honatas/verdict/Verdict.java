...
import io.github.honatas.verdict.exception.VerdictException;
...
public class Verdict {
	...
	private Logger logger = Logger.getLogger(Verdict.class.getName());
	...
	/** Creates a {@code Verdict} instance without data. */
	public Verdict() {
		this.logger.fine("Validator created without data");
	}

	/** 
	 * Creates a {@code Verdict} instance with data. 
	 * @param data the data to be validated
	 */
	public Verdict(Serializable data) {
		this.data = data;
	 ...
	 * @throws VerdictException if there are any errors
	 */
	public void checkHasErrors() throws VerdictException {
		if (this.hasErrors()) {
			throw new VerdictException(this.errors, this.data);
		}
	}
	...
	public interface Validator {
		 ...
		 */
		String validate(Object value, String name);
	}
	 ...
	 * @param name this will be used as the key on the errors map
	 * @param validators the {@code Validator}s that will act upon the data
	 */
	public void validate(Object value, String name, Validator... validators) {
		...
		this.logger.log(Level.FINE, "Validating field {0} with data {1}", new Object[] {name, value});
		for (Validator v: validators) {
			String message = v.validate(value, name);
	 ...
	 * 
	 * @param name name of the field in the object. This will also be used as the key on the errors map
	 * @param validators the {@code Validator}s that will act upon the data
	 */
	public void validate(String name, Validator... validators) {
		if (this.data == null) {
		...
		this.validate(value, name, validators);
	}

	private Object getFieldValue(String name) {
		try {
			return extractFieldValue(name);
		} catch (IllegalAccessException | InvocationTargetException | NoSuchMethodException | SecurityException e) {
			return null;
		}
	}

	private Object extractFieldValue(String name) throws IllegalAccessException, InvocationTargetException, NoSuchMethodException, SecurityException {
		try {
			String getterName = "get" + name.substring(0, 1).toUpperCase() + name.substring(1);
			Method getter = this.dataClazz.getMethod(getterName);
			return getter.invoke(this.data);
		} catch (NoSuchMethodException e) {
			Method accessor = this.dataClazz.getMethod(name);
